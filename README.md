...
### v2.0.0 (2026-05) — 機械学習 (ML) メジャー追加
- **🤖 機械学習 (ML) 機能を新規追加**: チャットUIから独立した管理画面 `/ml.html` で、データテーブル管理・SQL分析・PyTorch学習・推論まで一気通貫に提供
  - **データテーブル (DuckDB)**: CSV インポート、Web API インポート (JSON Path指定可、SSRF対策付き)、Python REST 経由の行追記 (`/ml/datasets/append`)
  - **SQL クエリ**: 読み取り専用 (SELECT/WITH のみ)、自動 LIMIT 1000行、DuckDB方言対応
  - **PyTorch 学習**: 回帰 (MLP) / 分類 (MLP) / 時系列 (LSTM) の3タスク、ROCm/CUDA GPU 自動検出
  - **自動前処理**: StandardScaler / LabelEncoder / 日時列の自動6特徴量分解 (year/month/day/dayofweek/dayofyear/is_weekend)
  - **ターゲットスケーリング**: 回帰/時系列で学習安定化、推論時に逆変換
  - **推論API + UI**: バッチ100件対応、カテゴリ列は select / 日時列は date picker、確率バー表示 (分類)
- **LLM チャット連携**: `ml_list_datasets` / `ml_describe_dataset` / `ml_query_dataset` / `ml_list_models` / `ml_predict` の5ツール追加
  - **MLプリフィルタ**: ユーザー発話に「具体的なテーブル名/モデル名」または「MLメタキーワード」が無い場合は、ML系ツールを `tools` 配列から自動除外 (雑談・コード生成での誤発動防止)
  - **派生列の自動復元**: LLM が `date_year, date_month, date_day` のような派生列を直接渡しても、サーバー送信前に元の日付文字列 (`"2026-04-20"`) に自動修正
  - **テキスト形式ツール呼び出しの救済**: LLM が `sales_yosoku{...}` のようなテキスト形式で書いた場合も、3パターンで検出して実ツール呼び出しに変換
  - **空応答救済**: ツール実行後の応答が空になった場合、理由説明モードで再生成、それでも空なら固定メッセージで通知
  - **`ml_list_models` に `predictHint` 追加**: 各モデルの正しい入力例 (exampleInput) と日時列の説明をLLMに渡す
- **API トークン認証**: `Authorization: Bearer <token>` 方式で外部プログラム (Python等) から API 直接利用可能、`ml.apiTokens` で権限分離 (ml:read / ml:write / *)
- **CORS 対応**: `/ml/*` エンドポイントは別オリジン (ブラウザJS) からも利用可能
- **DuckDB ロック調停**: Node 側で CHECKPOINT + 接続クローズ → Python 排他ロック取得 → 終了後再オープン (DuckDB 1.x の単一プロセス制限への対応)
- **ブランディング統一**: `OpenGeek` → `OpenGeekLLM` (タイトル、ロゴ、ログ)
- **UI 改善**:
  - チャット画面: 「🧠 ファインチューニング」「🤖 機械学習」を横並びボタンに
  - 機械学習画面: モデルカード右上に削除ボタン (×) を配置
  - 4タブ構成: 🗂️ データテーブル / 🔍 SQL クエリ / 🧠 モデル (学習) / 📡 API (外部連携)
- **⚠️ 破壊的変更 (BREAKING)**: config.json の構造を整理
  - 旧: `mlEnabled` / `apiTokens` (トップレベル)
  - 新: `ml.enabled` / `ml.apiTokens`
  - 旧形式の自動マイグレーションは廃止。手動移行が必要 (editconfig.html から修正可能)
- **依存追加**: Node.js `duckdb`、Python `duckdb` / `pandas` / `scikit-learn` / `torch`
- **🔧 外部API: ツール対応モード を新規追加** (`agent_proxy.js`):
  - 通常モード (素のLLM公開) に加えて、エージェント機能付きの OpenAI 互換 API を別ポートで提供
  - 対応ツール: ml_* (5) / web_search / list_files / read_file / search_documents (RAG)
  - 非対応: generate_image / Python実行 (セキュリティ理由)
  - ml_predict は派生列を自動復元、テキスト形式 tool_call も3パターンで救済
  - 起動時に内部 llama-server に指定モデルを自動ロード、アイドルアンロード後の再リクエストも自動再ロード
  - JSON パースエラー・404・予期しないエラーも HTML ではなく OpenAI 互換 JSON で返す
  - `/health` は認証スキップ (パブリック、ロードバランサー・監視ツール対応)
- **📚 永続RAGドキュメント (外部API専用)**:
  - uploads フォルダのテキストファイルを embedding 化して `ml/rag/` に永続保存
  - `POST /rag/documents`、`GET /rag/documents`、`DELETE /rag/documents/:id`、`POST /rag/search`
  - embedding ベクトル検索 (cosine類似度) で top-5 を返す
  - **embedding未設定時の RAG 自動 OFF (4層防御)**:
    1. UI: RAGチェックボックスが灰色 + 警告表示
    2. サーバー起動時: rag を要求しても自動除外 + 警告レスポンス
    3. API: `/rag/*` エンドポイントが 503 + 理由を返す
    4. agent_proxy: ツール一覧から自動除外
- **🔬 リファクタリング**:
  - **`ml_common.py` 新規作成 (Python共通モジュール)**: `classify_dtype` / `parse_datetime` / `expand_datetime_features` / `encode_value` / `example_input` を集約。学習 (ml_runner.py) と推論 (ml_predict.py) で重複していたロジック (95行) を排除、修正漏れリスクを解消
  - **RAG ロジック挙動統一**: ブラウザ側 `chunkText` / `cosineSim` とサーバー側 `ragChunkText` / `ragCosineSim` の差分 (cosine 分母 epsilon、ループ終了条件) を統一
  - **無限ループバグ修正**: `chunkText(text, 500, 600)` のように `overlap >= chunkSize` の場合に無限ループしていた問題を `safeOverlap = min(overlap, floor(chunkSize/2))` で対処
  - **`runMlPredict` 共通関数化**: 推論ロジックをエンドポイントと agent_proxy で共有
...
- **MarkdownContent の `React.memo` 化**: ストリーミング中の親 state 更新で過去メッセージが再レンダリングされる問題を解消。長いチャット履歴でも高速化、コードブロックの実行結果 (`output-*`) が破壊されない
- **コードブロック DOM ID の決定的生成**: marked の `renderer.code` で `Math.random()` ではなくコードテキストのハッシュから ID を生成。ストリーミング中の再レンダリングでも同じ ID が維持され、Python実行の結果表示が安定
- **Python実行機能のセットアップガイド充実**: matplotlib 日本語フォント自動検出、よくあるエラー一覧、必須・推奨パッケージのまとめを追記
